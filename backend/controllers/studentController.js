...
const getStudentsByCollegeId = async (req, res) => {
  const { id } = req.params;
  try {
    const students = await Student.find({ college: id }).populate(
      "college",
      "name"
    );
    res.json(students);
...
const getStudentsFromSameCollege = async (req, res) => {
  ...
  try {
    ...
    if (!collegeId) {
      return res
        .status(404)
        .json({ message: "No college found for the given user ID." });
    }
    const students = await Student.find({ college: collegeId }).populate(
      "college",
      "name"
    );
    ...
    logger.error(
      `Error fetching students from the same college: ${error.message}`
    );
    res
      .status(500)
      .json({ message: "Error fetching students from the same college." });
  }
};
