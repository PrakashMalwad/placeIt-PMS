...
exports.getMyCompany = async (req, res) => {
  try {
    const user = await CompanyCoordinator.findById(req.user.id).populate(
      "company",
      "name contactNumber"
    );
    ...
    res
      .status(500)
      .json({ message: "Error retrieving company: " + err.message });
  }
};
...
exports.getUserById = async (req, res) => {
  try {
    ...
    if (!user)
      user = await PlacementCoordinator.findById(req.params.id).catch(() =>
        PlacementCoordinator.findById(req.params.id)
      );
    if (!user)
      user = await CompanyCoordinator.findById(req.params.id).catch(() =>
        CompanyCoordinator.findById(req.params.id)
      );
    if (!user)
      user = await Admin.findById(req.params.id).catch(() =>
        Admin.findById(req.params.id)
      );
    if (!user)
      user = await PlacementCellAdmin.findById(req.params.id).catch(() =>
        PlacementCellAdmin.findById(req.params.id)
      );
    if (!user)
      user = await User.findById(req.params.id).catch(() =>
...
exports.deleteUser = async (req, res) => {
  try {
    let result;
    result = await Student.findByIdAndDelete(req.params.id).catch(() =>
      Student.findByIdAndDelete(req.params.id)
    );
    if (!result)
      result = await PlacementCoordinator.findByIdAndDelete(
        req.params.id
      ).catch(() => PlacementCoordinator.findByIdAndDelete(req.params.id));
    if (!result)
      result = await CompanyCoordinator.findByIdAndDelete(req.params.id).catch(
        () => CompanyCoordinator.findByIdAndDelete(req.params.id)
      );
    if (!result)
      result = await Admin.findByIdAndDelete(req.params.id).catch(() =>
        Admin.findByIdAndDelete(req.params.id)
      );
    if (!result)
      result = await PlacementCellAdmin.findByIdAndDelete(req.params.id).catch(
        () => PlacementCellAdmin.findByIdAndDelete(req.params.id)
      );
...
exports.getCollegeByUserId = async (req, res) => {
  try {
    ...
    user = await Student.findById(req.params.id)
      ...
      .catch(() => Student.findById(req.params.id));
    if (!user)
      user = await PlacementCoordinator.findById(req.params.id)
        .populate("college", "name contactNumber")
        .catch(() => PlacementCoordinator.findById(req.params.id));
    if (!user)
      user = await CompanyCoordinator.findById(req.params.id)
        .populate("college", "name contactNumber")
    ...
    res
      .status(500)
      .json({ message: "Error retrieving college: " + err.message });
  }
};
...
exports.holdUnholdUser = async (req, res) => {
  try {
    let user;
    user = await Student.findById(req.params.id).catch(() =>
      Student.findById(req.params.id)
    );
    if (!user)
      user = await PlacementCoordinator.findById(req.params.id).catch(() =>
    ...
    res
      .status(500)
      .json({ message: "Error toggling user status: " + err.message });
  }
};
