...
const updatePlacementStatistics = async (collegeId) => {
  try {
    ...
    const totalEligibleStudents = await Student.countDocuments({
      college: collegeId,
      isEligible: true,
    });
    const totalPlacedStudents = await Student.countDocuments({
      college: collegeId,
      isPlaced: true,
    });

    // Get placement coordinator by college ID
    const placementCoordinator = await PlacementCoordinator.findOne({
      college: collegeId,
    });
    ...
    if (!placementCoordinator) {
      console.error(
        `Placement Coordinator not found for collegeId: ${collegeId}`
      );
      return;
    }
    ...
    const placementRate = totalEligibleStudents
      ? (totalPlacedStudents / totalEligibleStudents) * 100
      : 0;
    ...
    console.log(
      "Placement statistics updated successfully for collegeId:",
      collegeId
    );
  } catch (error) {
